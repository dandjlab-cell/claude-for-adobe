...
function buildLedger(snap, transforms, { base = "V1", step = 0.5, half = 0.25 } = {}) {
  ...
  const cuts = [...edges.keys()].sort((a, b) => a - b).map((t) => {
    ...
    return { t, edges: edges.get(t), hiddenBefore: Number(before.covered.toFixed(2)), hiddenAfter: Number(after.covered.toFixed(2)), by: [...new Set(before.by.concat(after.by).map((b) => b.track + " " + b.name + (b.masked ? " (masked)" : "")))] };
  });
  const timeline = [];
  for (let t = 0; t < snap.duration; t += step) { const c = coverAt(rows, W, H, base, t + step / 2, hasAlpha); timeline.push([Number(t.toFixed(2)), Number(c.covered.toFixed(2))]); }
...
function visibleAt(ledger, t) {
  ...
  const hidden = ledger.cover[i] ? ledger.cover[i][1] : 0;
  const over = ledger.clips.filter((c) => c.track !== ledger.base && c.start <= t && t < c.end && c.share);
  return { hidden, baseVisible: 1 - hidden, over: over.map((c) => c.track + " \"" + c.name + "\" " + Math.round((c.share || 0) * 100) + "%" + (c.masked ? " masked" : "") + (c.graphic ? " graphic" : "")) };
}
