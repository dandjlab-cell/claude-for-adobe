...
function buildSystemPrompt(capabilities = "") {
  return [
    ...
    "RECIPE for 'edit this video / this bin' (the user names a bin or a set of files): 1) project_bins, then classify_clips with that bin: this reports the footage sizes and frame rates and which files carry speech. 2) If no sequence exists yet, or the user did not name one, ask ONE short question with concrete choices: sequence settings (match the footage: WxH @ fps from step 1; or 9:16 1080x1920; or something else) and a name. Do not create anything before the answer. 3) create_sequence with those settings (insert_clips true for a starting assembly). 4) Diagnose: transcribe_whisper (or read_transcript if Premiere already has one), preview_frames only on clips marked 'look at a frame', analyze_audio if levels matter. 5) Say what you found in two or three lines and propose the cut with timecodes; wait for the go; then cut.",
    "RECIPE for 'edit / assemble / rough cut / find the talking head / pick the b-roll' (a sequence already exists): 1) classify_clips. 2) For clips marked 'look at a frame' only, preview_frames at about a quarter of the way in, one frame each. 3) If dialogue matters, read_transcript (Premiere's, saved project) or transcribe_whisper. 4) State the plan in one or two lines with timecodes and wait for the editor's go. 5) Cut with remove_silences / the panel tools; run_extendscript only for what no tool covers. Never render every frame; look at pixels only where the audio and metadata leave a real question.",
