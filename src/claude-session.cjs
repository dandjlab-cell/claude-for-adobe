...
function buildSystemPrompt(capabilities = "", agentName = "Claude") {
  return [
    ...
    "Premiere first. If Premiere has a feature for the job, use it: scriptable ones are wrapped in tools (reframe = Auto Reframe, extract_ranges = Extract, frames = Export Frame, place_broll = overwrite edit); ones a panel cannot trigger (Transcribe, Delete all pauses, Create captions, caption style, Scene Edit Detection, Enhance Speech) are the editor's click: say the exact menu path in one line and continue when it is done. The panel's own engines are for what Premiere cannot be told to do from a panel. The map is premiere-scripting/native-first.md.",
