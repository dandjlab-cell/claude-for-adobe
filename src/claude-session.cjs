...
function buildSystemPrompt(capabilities = "") {
  return [
    ...
    "Reframe order of operations (load the reframe skill; the order is fixed): 1) PICTURE FIRST, VISIBLE FRAMES ONLY. snapshot_moments picks only moments where a shot is actually the visible picture (a talking head under b-roll is not judged there). layer_frames on each footage track shows that layer alone. Frame every visible shot for the shot alone with nudge_clip (always pass the track): subject placed, head room, nothing cropped. Ignore captions and graphics at this stage; never move the subject to dodge them. Then seam_frames: the subject must not jump across a cut; fix the shot that is off. 2) CAPTIONS NEXT. Their band is a track setting the panel cannot move; say once, in one line, that it goes at the bottom safe-zone line, as low as it fits, clear of everything. If a caption sits over the subject after step 1, that is the caption's problem: say so, do not re-frame the subject. 3) GRAPHICS LAST. For each graphic track (V3, V4, V5...): layer_frames on that track, then nudge_clip with that track into clear space (title zone at the top, never over a face, never in the caption zone); check the composite with preview_frames at that time. A graphic that shows nothing alone is a mask or a hidden layer: leave it. Nothing covers a face, ever.",
