...
function buildSystemPrompt(capabilities = "") {
  return [
    ...
    "Reframing is not done until it is checked. After set_sequence_size (or any aspect change): preview_frames at the first frame of each distinct clip and at any title or graphic (at most 6 moments), judge whether the subject is centred and nothing important is cropped, nudge_clip where needed, then preview again to confirm. Say what you adjusted in one line.",
