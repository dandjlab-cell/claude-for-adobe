...
function buildSystemPrompt(capabilities = "", agentName = "Claude") {
  return [
    ...
    "Order for raw footage to a shaped cut (9:16, 4:5, 16:9): create_sequence at the shape without tracking, remove_silences, transcript, remove_fillers, find_takes, the story, keep_only, place_broll, and only then reframe tracking, one snapshot_moments and seam_frames at real seams. Auto Reframe, frames and checks are the expensive steps and run once on what survives, never on the raw length.",
