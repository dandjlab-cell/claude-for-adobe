...
function createMcpServer({ name = "premiere", version = "0.1.0", tools, onCall, onLog = () => {} }) {
  ...
  // `signal` aborts when the client drops the request (the agent gave up waiting, or the panel stopped it), so a tool
  // that waits on the editor can cancel instead of acting later for nobody.
  function handle(message, signal) {
    ...
    if (method === "tools/call") {
      ...
      return Promise.resolve()
        .then(() => onCall(params.name, params.arguments || {}, signal))
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on("end", async () => {
      ...
      const response = await handle(message, abandoned.signal);
