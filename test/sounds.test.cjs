...
test("windows become merged segments per group with a peak", () => {
  ...
  assert.deepStrictEqual([laugh.start, laugh.end, laugh.peak, laugh.label], [0.5, 2.0, 0.55, "laughter"]);
...
test("report is one line per group", () => {
  assert.ok(/laughter \(1\): 0\.50s-2\.00s laughter 0\.55/.test(report(segments(parseWindows(text)))));
