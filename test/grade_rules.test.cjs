...
test("grade_sequence is wired, follows the rules, reuses the read's region on the confirm", () => {
  ...
  assert.match(seqTool, /if \(temp && !ref && !next\.highlights\) \{/, "the correction scales the white balance from the real reading, unless the Highlights pad was already corrected or the colour is matched from an earlier cut of the same file");
  // Shot match (2026-09-16): a later cut of a graded file takes the first cut's COLOUR and solves its own tone.
  assert.match(seqTool, /const temp = ref \? matchedBalance\(m, ref, tempFrom, tintFrom\) : gradeTemperatureFor\(m, tempFrom, tintFrom\);/, "the white balance of a later cut is the first cut's");
  assert.match(seqTool, /padSolved = ref \? \{\} : pads\.wheels/, "a matched colour is not corrected per cut");
  assert.ok(seqTool.indexOf("gradeLevelsFor(afterBalance, currentCurves, m)") > 0 && !/ref \? [^\n]*gradeLevelsFor/.test(seqTool), "the curve is solved per cut, matched or not");
