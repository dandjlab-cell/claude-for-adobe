...
The **Cut silences button** builds a separate sequence from retained source ranges, inserting linked video/audio pairs in native batches of 24. A failed insertion can retry at 16 then 8 only when the source and last verified output remain intact; crashes or ambiguous changes stop the job. It preserves the original sequence dimensions and static clip scale/position, and checks the resulting source and sequence boundaries. Stop keeps an explicitly incomplete copy. This first version supports contiguous normal-speed linked V1/A1 footage with static intrinsic effects; unsupported layouts are refused. Because this Premiere scripting API cannot inspect caption tracks, the button asks you to confirm that the sequence has no captions before proceeding. The chat `remove_silences` tool and editorial extraction tools still use their existing paths.

The button saves a `.silence-rebuild.json` report beside the project: verified source ranges, original timeline ranges, and new sequence ranges. When a matching original transcript exists, its word metadata is mapped into that report and words crossing cuts are flagged. This report is not a newly transcribed or audibly verified transcript.
...
`list_analysis` defaults to the selected Project clips, selected bins, or otherwise the active timeline. It returns a compact list of matching analysis filenames and project-guidance titles; unrelated edits, chats, renders and debug files stay out. `all:true` explicitly lists the full project inventory. Legacy filenames are discovery candidates, not proof that an analysis belongs to the current cut; identity still needs checking. Unscoped old notes are not automatically treated as approved project rules.
