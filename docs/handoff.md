...
1b. **Caption band position** is the one step in the order the panel cannot do (the user wants captions dropped to the bottom safe-zone line). It is not in the ExtendScript surface we use. Investigate the QE DOM with a read-only enumeration from the panel (Claude can run it; it will ask for a click): `app.enableQE(); var q = qe.project.getActiveSequence(); var out = []; for (var k in q) { if (/caption/i.test(k)) out.push(k); } out.join(",")`. Also worth a look: CEA-608 `.scc` import honours row placement, which would let `create_captions` bake the position, at the cost of 608 styling limits.
