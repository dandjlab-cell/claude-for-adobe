...
1b. **Caption band position: SOLVED through the project file (commit 3ffb487), not yet run inside Premiere.** `caption_style` (y offset, size) rewrites every caption's FlatBuffers style block in the saved `.prproj` (fields by vtable slot: y = root/0/33/2 float, zone = root/0/5 int with 0 top 1 middle 2 bottom, size = root/0/0/[0]/1/1 float), then host `reloadProject` closes and reopens the same project. A/B fixture projects live outside the repo in the ASI-Evolve tests folder; `test/prproj.test.cjs` runs against them when present. First live test: on the dev panel say "move the captions down to the safe zone"; watch that the project reopens with the same sequence active and that host events still arrive afterwards (if `pendingChanges` stop or double, re-bind events after reload). Zone changes relayout the table and are not written.
