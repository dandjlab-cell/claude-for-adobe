...
**Last commit:** `167bb8e` handoff: 10/18 balanced at 23:08 (the code is at `78e1954`)
**Role:** BUILDER
...
**Suite:** `node --test test/*.test.cjs` → **294 tests, 293 pass, 1 skip** (`whisper.test.cjs` fetches `schemas.adobe.com`; the skip is counted, not a failure). The privacy scan is in the suite and in the pre-push hook.
...
- **Speed question open:** a Premiere render went from 0.8 s to 3 s over the evening and the source read from 1.6 s to 3 s with no change in the pass (the user deleted the working copies each time, so it is not those). Either Premiere's session state after ~1000 effect writes or the heavier effect stack (curves + wheels + sliders). **Test: restart Premiere, run once, read the timing column at the end of each row** (`[Ns: read a, renders b, N host calls, rest c]`). ~1 s a render = session; ~3 s = the stack's real cost.
- **Not yet run live:** `78e1954`'s last change (relative ceiling check in the correction) ran in the 23:08 run; nothing after it changed code. The handoff commits are docs only.
...
1. **Restart Premiere, reload the panel (footer must show `dev 167bb8e` or later), discard any copy, run `grade this video` once**, and read the timing column. Done when: the run's rows show a per-render time, and the result replaces the "Speed question open" bullet under "What's in progress" in this file, stating ~1 s (session state) or ~3 s (the effect stack's cost). Then leave the balance pass alone on this footage.
2. **Luma vs Sat probe** (Hue Saturation Curves): read-only check whether the QE text door (`lumetriQE` by name) writes it like RGB Curves. If yes, add the colourists' cleanup after the balance: roll saturation off in the deepest shadows and near-whites (Frame.io, the Resolve manual, a Premiere user's default preset). Never on a coloured surface — it drains it. Done when: a read-only QE probe (`getParamValue` on the Hue Saturation curve names) has confirmed or denied a text form, and if confirmed, one live write + scopes read shows the ends desaturating while the median saturation holds.
