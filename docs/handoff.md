...
**Suite:** `node --test test/*.test.cjs` → **309 tests, 308 pass, 1 skip** (`whisper.test.cjs` fetches `schemas.adobe.com`; the skip is counted, not a failure). The privacy scan is in the suite and in the pre-push hook.
