...
- **The color parade's ends** — the darkest and brightest few percent of pixels taken together, so the reading reflects real parts of the picture rather than three unrelated extremes.
- **Casts by brightness band** — whether shadows, midtones or highlights lean warm, cool, green or magenta.
- **The vectorscope reading** — color angle and strength, used to judge skin tone.
...
The pass does not invent its own idea of a good picture; its targets come from standard color-correction references and broadcast convention, cited in the panel's internal color notes.
...
1. **White balance** — Temperature, then Tint, solved from the brightest 1% of the picture: usually a shiny highlight reflecting the room's actual light, a better reference than the brightest 3%, which can just be a light-colored surface.
2. **The Shadows and Highlights wheel pads** — small color nudges cleaning up whatever cast is left at the dark and bright ends after the white balance.
...
4. **The Luma vs Sat roll-off** — a small saturation reduction in the deepest shadows and near-whites, so noise there doesn't carry a visible color cast.
...
White balance and the wheel pads happen before the black point and the sliders. This is because the color-cleanup tools read the darkest and brightest pixels; once the black point has already been pulled down, those same pixels are pinned at the floor of the scale and can no longer be read properly.
...
- **A shot's own color is not a light cast.** If a parade end stays warm or cool by more than about 20 points even after white balance, that is treated as the object's own color and left alone — one test shot read warm by 25–32 points across nearly every brightness band with only its shiny highlights near neutral, and neutralising it would have drained the warmth out of the objects.
- **A colored parade end gets no wheel pad.** The same 20-point rule applies per wheel: a pad can only part-neutralise an object's color, and tints whatever the black-point curve has already crushed underneath it.
...
- **A shared cast at both ends is the room, not the light.** If shadows and highlights lean the same way by more than about 20 points, that is the scene's own color and no white balance is applied, stated in the result. Forcing one on an oak-table shot once drained it to pale grey-beige and washed out the skin.
- **A strongly colored top over shadows leaning the other way is an object, not the light.** The brightest pixels of that same oak shot are the wood's sheen, warm by 25, while a blue cloth in its shadows reads cool. Splitting the difference made the whole shot pale, so a top more than about 20 points off neutral gets no white balance whatever the shadows do; the pads take the shadows.
...
If applying the shared grade to a later cut would clip, crush, or push its white point too close to clipping, the whole shot's tone is turned down by half across every cut, including ones already graded, and all are rewritten to match. This exists because cuts of the same shot, seconds apart, must never end up looking visibly different in color from each other — brightness may vary a little; color should not.

## 7. Skin

Any clip where Vision found a face or a hand gets a step aimed at skin tone. The tool is a hue curve, not a color key, and that choice follows what colorists actually do.

- **Why a curve.** The working hierarchy is primaries first, then curves, then Hue vs Hue, and only then a color key using as few controls as possible. The key is the last resort, not the first. A curve also needs no key, no mask and no tracking, which matters for an automatic pass.
...
- **Why the key was abandoned.** On footage where the surroundings share skin's color, a wooden table or cream cabinets, no key holds the skin without also holding the room. Narrowing it until it does catches only the rims of a hand, and coloring a rim is the worst outcome available.
...
- **The color wheels' brightness control is not used**, even though it has now been measured: its response is uneven across the tested footage, sometimes darkening a subject overall instead of cleanly lifting shadows, so a dark-subject lift still uses the ordinary Shadows slider for now.
...
- **On this footage the skin step rarely acts.** In the run of 2026-09-16 at 15:09, three clips were already on the skin line and needed nothing, four reported that no key separates the skin from the room, and none were corrected. The step is proven to work end to end, on earlier runs where a key did separate, but a color key is the wrong tool for a hand on a wooden table. The right tool there is a shape mask around the face or hands, which Lumetri supports and which Vision already locates. That is not built.
