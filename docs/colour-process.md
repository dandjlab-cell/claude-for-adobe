...
Any clip where Vision detected a face or a hand gets an extra step aimed at skin tone, using Lumetri's HSL Secondary tool, which can select a specific range of colour (a "key") and grade only that.

- **Learning the key.** The pixels inside Vision's face or hand boxes that Vision's person mask also marks as part of a person are the sample. They are a seed, not the key: a key cut to the sample's own range catches only the rims of a hand. Following the published method, the key starts wide on hue, open at the bottom of saturation and fully open on lightness, and is then narrowed against this frame, one step at a time, always the step that removes the most background for the least skin.
- **When no key works.** The narrowing stops when the key is acceptably tight or too little skin is left. On footage where the surroundings share skin's colour, a wooden table or cream cabinets, it never gets tight enough, and the pass says so in the row and writes nothing. On the sandbox that is four clips of eighteen.
- **The spill check.** How much of the frame the key would light is computed from the frame itself, not from Lumetri's mask view: four attempts never got that view into a rendered frame, whatever the wait. Checked by hand once, the computed figure and Lumetri's own mask agreed inside the rule's margin.
- **Refine before correcting.** Denoise and Blur soften and de-speckle the selection. They are written before any colour move, because a loose blurred key takes a correction cleanly where a hard one bands.
- **The Midtones wheel rotation.** Once the key is trusted, the correction is a rotation of Lumetri's Midtones colour wheel, applied only inside the key, turning the selected colour's hue toward the skin line. An earlier version used HSL Secondary's own Tint control, but that washed every keyed pixel toward magenta regardless of the shot, so it was replaced with the wheel rotation.
- **Confirming and rescaling.** After the wheel write, the pass renders once more and re-measures the skin region, then rescales the correction from how far the hue actually moved, repeating until it lands. The wheel is about six times stronger on some keys than others, so the first write is always a guess and the clip's own response is what lands it. A correction that turned the hue the wrong way is removed.
- **Saturation only comes down.** Pale skin is never pushed up. Doing so amplifies whatever tint the skin already carries and turns it pink, which is what happened on the first live run. Pale skin is an exposure and white-balance problem, not a saturation one.
