...
- **Learning the key.** Rather than an editor eyedropping a patch of skin, the key is learned from the pixels inside Vision's face or hand boxes that Vision's person mask also marks as part of a person, filtered by a colour prior for skin so a ring, a sleeve or the table under the hand doesn't teach it. The key is then widened a little past the pixels' own range, because Lumetri's idea of hue, saturation and lightness does not match this measurement to the decimal and a key at the exact range left parts of an arm out.
- **The spill check.** With the key built, the pass turns on Lumetri's "Show Mask" view, which flattens everything unselected to grey, and measures how much of the frame is coloured against how much the Vision boxes cover. A key that grabbed far more than the boxes — a kitchen's cabinets alongside a hand, say — is tightened once and re-checked; if it still spills, skin grading is skipped on that clip and the panel says so.
- **The thin-key check.** If the key lights up far less of the frame than the boxes' own skin content suggests it should, it is flagged as too narrow.
...
- **Confirming and rescaling.** After the wheel write, the pass renders once more and re-measures the skin region, then rescales the correction from how far the hue actually moved, since the wheel's measured response does not transfer identically to every key. A correction that turned the hue the wrong way is removed rather than left in place.
...
- **The skin step has completed one live run** (2026-09-16, 14:12): every keyed clip ended on the skin line after the measured correction. But the first write overshot on every clip because the wheel had been calibrated on a much thinner key, and the spill check could not read the mask view because Premiere handed back the frame it had drawn before the mask switch. Both are corrected in code and await the next run.
