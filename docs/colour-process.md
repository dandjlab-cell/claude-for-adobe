...
- **The skin step has not yet completed a live run.** Its pieces are each proven by hand (the key text, the mask view, the wheel's response), but as of this writing every full pass has stopped before the wheel write, first on keys that took the whole frame, then on a mask read that came back before the mask rendered.
