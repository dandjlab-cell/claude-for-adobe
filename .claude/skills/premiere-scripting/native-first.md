...
| Remove pauses | Text panel > Delete all pauses (0.75 s) | cannot be triggered | `remove_pauses` (same definition), `remove_silences` (voice detection) | Ours: same numbers, one History step per range. |
| Remove ums, stutters, repeats | none | | `remove_fillers` | Ours. |
...
| Caption band position / style | Captions track style, Essential Graphics | not scriptable | none | Editor's click. Say where it should sit; continue. |
