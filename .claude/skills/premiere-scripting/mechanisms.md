...
| Two takes of the same line in the footage | none in Premiere | `find_takes` (utterances from the transcript, content-word overlap within a window, most complete take kept: words, fillers, finished ending) | whether the "complete" take is also the better delivery; prosody per take is the next input |
...
| A folder of raw footage and a shape or length ("make a 9:16 video", "a 20 s cut") | `create_sequence` at the shape WITHOUT tracking (talking head bin only), `remove_silences`, transcript (Premiere's or `transcribe_timeline`), `remove_fillers`, `find_takes`, `keep_only` to length, `place_broll`, and only then `reframe` tracking on the survivors, one `snapshot_moments`, `seam_frames` at real seams, captions last. Tracking and frames are the expensive steps: never on the raw length | which lines carry the story and in what order, which b-roll covers which line |
