...
| What the viewer sees at a moment (how much of V1 is hidden, by what) | Premiere's own clip data: Motion, Crop, Opacity, source size vs frame, Video Info Alpha, mask presence | verified reads; `visible_at` and the visibility ledger (`<seq>.visibility.json`, rebuilt on every timeline change) | none: it is a lookup; masks are the one case where a frame render settles the area |
| Laughs, applause, reactions, music on the timeline | none in Premiere | `sound_events` (Apple's 303-class sound classifier over Premiere's own WAV render; segments per group with peak confidence; cached per cut). Verified on real audio 2026-09-07 | which reaction becomes a cut, which pause is a beat |
