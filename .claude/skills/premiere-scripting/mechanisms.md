...
| A folder of raw footage and a shape or length ("make a 9:16 video", "a 20 s cut") | `rough_cut` (one call, fixed order: sequence at the shape without tracking, silences, transcript, fillers, takes; stops at the story), then `keep_only` to length, `place_broll`, `reframe` (no bin) once for tracking and checks, captions last. Tracking and frames are the expensive steps: never on the raw length | which lines carry the story and in what order, which b-roll covers which line |
