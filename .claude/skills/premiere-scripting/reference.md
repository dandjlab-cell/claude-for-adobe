...
`q.getVideoTrackAt(i)` / `q.getAudioTrackAt(i)`, `qt.isSyncLocked()`, `qt.setSyncLock(bool)`, `q.CTI.timecode`, `qeClip.addVideoEffect(...)`, `q.exportFramePNG(...)`. Every other QE name on this build is in `surface-26.3.2.md`; listed there means it exists, not that it has been run.

## Can / cannot, by task

| Task | Answer | Exact call | Trap |
|---|---|---|---|
| List clips with times | yes (panel) | loop `seq.videoTracks[t].clips[c]`, read `.name`, `.start.ticks`, `.end.ticks`, `.inPoint.ticks`, `.projectItem.getMediaPath()` | Wrap `getMediaPath` in try: synthetic items (color matte, adjustment layer) throw. Cap output; the panel truncates at 64 KB. |
...
| Sequence markers | yes (docs, note) | `var m = seq.markers.createMarker(seconds); m.name = ...; m.comments = ...; m.end = seconds + dur; m.setTypeAsComment(); m.setColorByIndex(i, 0)` | Color indexes per docs: 0 green, 1 red, 2 purple, 3 orange, 4 yellow, 5 white, 6 blue, 7 cyan (the note uses a different list; read back with `getColorByIndex()`). Assign `m.end` a seconds Number, not a Time. |
...
| Add a track | QE, listed | `q.addTracks(...)`, `q.removeEmptyVideoTracks()`, `q.removeEmptyAudioTracks()` exist on 26.3.2 (`surface-26.3.2.md`); arguments unverified | `importMGT` and `insertClip` need the target track to exist. Ask the user to add one, or use a track that exists. |
...
| Remove a time range (extract) | yes (panel) | `seq.setInPoint(a); seq.setOutPoint(b); app.enableQE(); qe.project.getActiveSequence().extract()` | Prefer `extract_ranges` / `keep_only`. Process ranges in descending time order, target and unlock every track, sync-lock via QE so audio follows video, snap to frame edges, and restore in/out afterwards. Expect occasional one-frame holes. NEVER set the out point on the sequence's last frame: Extract then removes the whole timeline (reproduced 2026-09-05); a range that reaches the end is a tail trim (remove clips after `a`, set the spanning clip's `.end`). Second trap (2026-09-05, isolated on five fresh copies): for particular out values `setOutPoint` **silently clears the in point to zero**, so Extract takes everything from the head. The same start with a different out is clean, one frame later is clean, so it is the out value itself. Set the out first, then the in, read both back with `getInPoint()` / `getOutPoint()`, re-set whichever drifted, and never extract unless both hold. **Found (2026-09-07, from the Extract trace):** every range extracted exactly; the wipe was the gap-closing pass, which set an in point on a one-frame hole and Premiere reset it to zero, so Extract ran from the head. Never Extract a sub-frame or one-frame hole. Holes are now closed by extending the previous clip's `.end` to the next clip's start (video and audio), no in/out points involved; a bad extract is still undone through `qe.project.undo()` before stopping. |
...
| Speed / duration change | QE, listed | `clip.getSpeed()` is read-only; QE `qeClip.setSpeed(...)`, `setReverse`, `setTimeInterpolationType`, `setFrameBlend` exist on 26.3.2; read `qeClip.speed` back | Tell the user; do not loop on QE variants. |
| Transitions | QE, listed | `qeClip.addTransition(qe.project.getVideoTransitionByName(name), ...)`; 144 names incl. Morph Cut in `surface-26.3.2.md` | Say it is unreliable; the user can apply it in the UI. |
| Effects | QE (Auto Reframe verified; others listed) | `qeClip.addVideoEffect(qe.project.getVideoEffectByName(name))`, `addAudioEffect`; params by `qeComponent.getParamList` / `setParamValue`; names in `surface-26.3.2.md` | Existing intrinsic components (Motion, Opacity, Volume) are editable without QE. |
| Split / razor a clip | QE, listed | `q.razor(time)` / `qeTrack.razor(time)` exist on 26.3.2; ExtendScript has none | Workaround: overwrite the same project item twice with different in/out points, then remove the original. Usually better to ask the user to razor. |
| Move a clip to another track | QE, listed | `qeClip.moveToTrack(...)` exists on 26.3.2; ExtendScript has none | Overwrite on the new track, remove from the old. |
| Playback | QE, listed | `qe.startPlayback()` / `stopPlayback()`, `app.sourceMonitor.play(speed)`; ExtendScript has `setPlayerPosition` only | |
