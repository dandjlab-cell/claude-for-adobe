...
1. **The grade asks; it never converts log on its own.** `grade_sequence` names the log clip, names the maker from the file's tags, and stops there. Put the choice to the editor in one short question, with the cost of each: Premiere's own conversion is exact and covers every cut of the file, but it is a **source setting that stays after Discard copy**; the maker's LUT sits on the clip and goes away with Discard. Then run `grade_sequence` again with `log: "interpret"` or `log: "lut"` - the conversion goes on first and the grade follows in the same pass. Never pick for them, and never set the interpretation without being asked to.
2. **Premiere's own conversion, chosen from the picture** (`log: "interpret"`). It tries the maker's log colour spaces, renders each, scores the result as a display picture and keeps the winner; the row reads `log → Sony S-Log3/S-Gamut3.Cine (a source setting, at your word; …)`. Tone and gamut both, no file needed.
3. **The maker's official LUT, in one call.** If the editor prefers the maker's own LUT, or the conversions all failed, ask once: "This is Sony log. I can download Sony's official conversion from their site and put it on the clip. Shall I?" Name the maker and the site, never search the web, and never make the editor find an id or a folder. With `log: "lut"` the grade does this itself for every log clip. On its own, `log_lut use` at the clip's time does the lot: works out the maker from the file, downloads if it is not already here, applies it to that clip, and reports what the render now reads. It lives on the clip, so Discard copy removes it. `status`, `fetch`, `apply` and `clear` exist for the awkward cases; `use` is the normal one.
