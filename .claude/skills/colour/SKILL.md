...
1. **The grade asks the editor itself, with buttons. Do not ask in chat.** At the first log clip `grade_sequence` puts two questions in the message stream as clickable answers (the same card `ask_user` gives you), and carries straight on with whatever is clicked - no second call, no rerun. Just call `grade_sequence` and relay the result. Writing the question out in prose is the old way and reads as a wall of text; the buttons are the interface, here and anywhere else you need an answer.
   - **Which conversion?** the maker's official LUT (downloaded from their own page) · the LUT Premiere already ships · leave it log. Premiere's bundled set covers **Sony, ARRI and RED only** - for any other camera the question says so and the official LUT is the only conversion offered.
   - **Where does it go?** the file's **source settings** (every cut of the file, stays after Discard copy) · **this clip's Lumetri Input LUT** (goes with Discard copy).
2. **Those are the only two choices.** Which file, and where it goes. Don't invent a third: "Premiere's built-in transform" means nothing at an editor's desk (the owner, 12:45). Premiere's colour-management transform still exists as `log: "premiere"` - a source setting, no LUT file, chosen from the picture by trying the maker's log spaces and scoring the render - but it is not one of the buttons and is only worth naming if the LUT routes fail.
3. **What each `log` value does, if you ever need to pass one.** `lut` / `lut-source` = the maker's official LUT on the clip / in source settings. `builtin` / `builtin-source` = the same two places with Premiere's own file. `skip` = leave it. The conversion goes on first and the grade follows in the same pass. On its own, `log_lut use` at the clip's time does the lot for one clip: works out the maker from the file, downloads if it is not already here, applies it, and reports what the render now reads. `status`, `fetch`, `apply` and `clear` exist for the awkward cases; `use` is the normal one.
