...
1. **The grade asks the editor itself, with buttons. Do not ask in chat.** At the first log clip `grade_sequence` puts two questions in the message stream as clickable answers, and carries straight on with whatever is clicked - no second call, no rerun. Just call `grade_sequence` and relay the result. Writing the question out in prose is the old way and reads as a wall of text; the buttons are the interface.
