...
1. **Always the maker's own official LUT.** Premiere's bundled LUTs are not an alternative and are never offered - the camera "profiles" in Lumetri's menu are a look, not a conversion, and they are not good (the owner, 13:10). Every conversion is fetched from the maker's own page, and the panel keeps the links so nothing is ever searched for on the web.
2. **The grade asks the editor itself, with buttons. Do not ask in chat.** At the first log clip `grade_sequence` puts **one** question in the message stream as clickable answers (the same card `ask_user` gives you) - it names the camera, names the site the LUT comes from, and asks only where it goes: **this clip's Lumetri Input LUT** (goes with Discard copy) · the file's **source settings** (every cut of the file, stays after Discard copy) · **leave it log**. It then carries straight on with whatever is clicked - no second call, no rerun. Just call `grade_sequence` and relay the result.
