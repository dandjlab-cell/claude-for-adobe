...
8. **Cut, social-tight by default.** Unless the editor asks for a looser feel, cut like a social edit: almost no air. `remove_silences` with `method: "vad"`, `min_silence_s: 0.35`, `pad_s: 0.05` (the same engine as the Cut silences button, just tighter). Dry run, one line, go, apply. Then the panel tools for moves; `run_extendscript` only for what no tool covers. Say what changed, the new duration, and how to undo it.
