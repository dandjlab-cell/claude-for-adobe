...
1. `reframe` with the aspect asked for, on the open timeline (never with a bin: raw footage in a bin is `rough_cut`'s job, and the tracking pass on its cut is `reframe` without a bin). One call: the open timeline is resized on its working copy, footage fills and is centred, graphics keep their placement, then Premiere's own Auto Reframe effect goes on every footage clip so Premiere follows the subject from the clip's source. It returns the visible moments and the seams with CHECK lines. `motion: "static"` skips the tracking. For "check the framing" with no shape change, start with `snapshot_moments` (inspection, nothing moves) and continue from step 2. (`set_sequence_size`, `snapshot_moments`, `seam_frames` are its parts; use them alone only for a re-check.)
...
3. For each footage track that has clips (V1, V2...): first `subject_path` with that track. It is Premiere's own answer to where the subject is, per key, in timeline time (x,y frame fractions; 0.5,0.5 is centre). Read it: a y that sits high or low for the shape, or a drift across a clip, is a fact, not a guess. Then `layer_frames` with that track to see the layer alone and confirm. Fix with `nudge_clip` and always pass the track. Look again.
...
6. Looking is a loop of questions. Start coarse (`snapshot_moments`); each frame either answers or raises a question (cropped for the whole clip or just here? does this graphic move? is the subject drifting?), answered by looking closer (`frames_across` on that clip or those seconds), never by assuming. Decide only from frames you have seen. `visible_at` is the ledger of what covers what at any time and every cut; use it before holding on a face, covering a line or putting a transition on a cut, and never infer cover from a frame when the ledger knows. Its two numbers (hidden for certain, possibly under an alpha layer) are not a contradiction: settle open ones with `settle: true`.
7. Reframing is a loop, not a report: look, fix, look, up to four passes per moment. Never say something "will want repositioning": reposition it. Never ask left-or-right when you can see the frame; ask only when two subjects compete for it. Before saying where something was, read it (`clip_transforms`); before reversing yourself, read it again. Report in one line when everything checks out.
