...
1. `reframe` with the aspect asked for, and `bin` when building from raw footage. One call: from a bin the sequence is built straight at the asked shape (never an intermediate shape); an open timeline is resized on its working copy. Footage fills and is centred, graphics keep their placement, then Premiere's own Auto Reframe effect goes on every footage clip so Premiere follows the subject from the clip's source. It returns the visible moments and the seams with CHECK lines. `motion: "static"` skips the tracking. (`set_sequence_size`, `snapshot_moments`, `seam_frames` are its parts; use them alone only for a re-check.)
