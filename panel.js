...
async function curveSweepTool({ seconds, track = 1, points = null, curve = "Master", lift = false, anchor = null, hue = 0 } = {}) {
  const at = Number(seconds);
  ...
  } catch (error) { try { if (before) await cw.write(before); } catch (_) {} return err(card, error.message); }
