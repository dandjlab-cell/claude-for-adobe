...
const { REGISTRY: LUT_REGISTRY, HOME: LUT_HOME, forMaker: lutsForMaker, isLocal: lutIsLocal, localPath: lutLocalPath, fetchLut, builtinFor: lutBuiltinFor } = require(path.join(extensionRoot, "src", "luts.cjs"));
...
async function logLutTool({ action = "status", id, seconds, track = 1, where = "clip", which = "official" } = {}) {
  ...
  try {
    ...
    if (action === "use") {
      ...
      // Two sources for the file: the maker's official one (downloaded) or the one Premiere already ships.
      const builtin = which === "builtin" ? lutBuiltinFor(hint) : null;
      if (which === "builtin" && !builtin) return err(card, "Premiere ships no built-in conversion for " + hint + " - its bundled set covers Sony, ARRI and RED only. The maker's official LUT is the way for this footage (which=\"official\").");
      const mine = builtin ? [{ id: null, label: builtin.label, maker: hint, here: builtin.path }] : lutsForMaker(hint);
      ...
      const notes = [];
      for (const l of mine) {
        let p = l.here || lutLocalPath(l.id);
        if (!l.here && (!p || !fs.existsSync(p))) {
        ...
        if (moved) {
          const text = "Using " + l.label + (where === "source" ? " in the file's source settings (stays after Discard)" : " on this clip (goes with Discard)") + ". It reads black " + round2(after.luma.p1) + " / white " + round2(after.luma.p99) + " / colour p99 " + round2(after.saturation.p99) + ", where it read " + round2(before.luma.p1) + " / " + round2(before.luma.p99) + " / " + round2(before.saturation.p99) + " as shot. The file is " + (l.here ? "Premiere's own, inside the application" : "in " + LUT_HOME + "/" + l.maker + " and stays there") + "; " + (where === "source" ? "the setting is on the file, so it survives Discard copy" : "the LUT is on the clip, so Discard copy removes it") + "." + (notes.length ? " (" + notes.join("; ") + ")" : "");
          card.done(text, true); return { text };
        }
  ...
  for (const c of clips) {
    ...
    if (readFrom === "source" && !graded && gradeLooksLikeLog(m)) {
      ...
      if (/^(lut|builtin)(-source)?$/.test(log)) {
        ...
        const r = await logLutTool({ action: "use", seconds: at, track, where: toSource ? "source" : "clip", which });
        if (r.isError) { lines.push(label + ": " + logNote + ". " + r.text); logSkipped++; continue; }
        logPart = "log → " + (which === "builtin" ? "Premiere's own built-in LUT" : "the maker's official LUT") + ", " + (toSource ? "in the file's source settings (stays after Discard)" : "on this clip (goes with Discard)");
