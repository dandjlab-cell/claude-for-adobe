...
const { cameraHint: logCameraHint, declaredSpace: logDeclaredSpace, candidates: logCandidates, pick: logPick, ACCEPT: LOG_ACCEPT } = require(path.join(extensionRoot, "src", "logspace.cjs"));
const { REGISTRY: LUT_REGISTRY, HOME: LUT_HOME, forMaker: lutsForMaker, isLocal: lutIsLocal, localPath: lutLocalPath, fetchLut, site: lutSite, resolve: lutResolve } = require(path.join(extensionRoot, "src", "luts.cjs"));
...
// A button is something to choose, not a paragraph to read, and nothing enforced that until now (the owner,
// 13:35). The longest answer we write is 29 characters and the longest hint 45, so these caps sit above what
// a good answer needs and below what wrecks the card in a narrow dock. ask_user refuses past them - it is
// the one caller we do not write ourselves - and askChoice clips, so no caller can break the card.
const ANSWER_MAX = 32, HINT_MAX = 90;
const clipTo = (s, n) => (String(s).length > n ? String(s).slice(0, n - 1).trimEnd() + "\u2026" : String(s));
...
function askChoice(text, answers, signal = null) {
  return new Promise((resolve) => {
    ...
    for (const a of answers) {
      ...
      const strong = document.createElement("b"); strong.textContent = clipTo(label, ANSWER_MAX); b.appendChild(strong);
      if (hint) { const s = document.createElement("small"); s.textContent = clipTo(hint, HINT_MAX); b.appendChild(s); }
...
async function askUserTool({ question, options = [] } = {}, { signal } = {}) {
  ...
  const tooLong = answers.find((a) => a.label.length > ANSWER_MAX) || answers.find((a) => a.hint.length > HINT_MAX);
  if (tooLong) return { text: "CLAUDE_FOR_ADOBE_ERROR:an answer is a button, not a sentence: keep label under " + ANSWER_MAX + " characters and description under " + HINT_MAX + ". Too long: \"" + (tooLong.label.length > ANSWER_MAX ? tooLong.label : tooLong.hint) + "\". Put the detail in the description, or shorten it.", isError: true };
...
async function chooseLogConversion(at, track, mediaPath, region, timed, onRender) {
  ...
  const tags = mediaTags(mediaPath);
  const hint = logCameraHint({ tags, path: mediaPath });
  // The file's own declaration first: a tagged log file names its curve, and then nothing is rendered to
  // find out (the owner, 12:25). Only an untagged file goes to the picture test.
  const declared = logDeclaredSpace(tags);
  if (declared && names.includes(declared.space)) {
    const r = await host("setColorSpace", String(at), String(track), declared.space);
    if (r.indexOf("ERR:") !== 0) {
      const m = await timed(() => measureFrameAt(at, { region, keepPlayhead: true }), "render"); onRender();
      return { name: declared.space, m, rows: [], score: 0, tried: 0, declared: true, hint: declared.maker, current, original };
    }
  }
...
async function logLutTool({ action = "status", id, file, seconds, track = 1, where = "clip" } = {}) {
  ...
  try {
    ...
    if (action === "apply" || action === "clear") {
      ...
      let lutPath = "";
      // A file the editor named wins over an id: their own LUT, a client's, or one of Premiere's bundled
      // ones if they ask for it by name. The panel never offers those - it does not refuse them either
      // (the owner, 13:26: "if the user wants a built in option they can, but it's not something we offer").
      if (action === "apply" && file) { lutPath = lutResolve(file); if (!lutPath) return err(card, "no LUT file called \"" + file + "\" - give a full path, or the name of one of Premiere's own (they live in the application's Lumetri/LUTs folders)"); }
      else if (action === "apply") { if (!id || !LUT_REGISTRY[id]) return err(card, "id must be one of: " + Object.keys(LUT_REGISTRY).join(", ") + " - or pass file with a path or a LUT's name"); lutPath = lutLocalPath(id); if (!lutPath || !fs.existsSync(lutPath)) return err(card, "not on this machine yet: fetch " + id + " first"); }
...
function hueCurveWriter(at, track, name) {
  const call = async (value) => {
    const raw = await host("lumetriQE", String(at), String(track), name, value);
    if (raw.indexOf("ERR:") === 0) throw new Error(raw.slice(4));
    const [, text, clipName] = raw.split(COL);
    return { points: parseSatCurve(text), text, clipName };
  };
...
// CALIBRATION, not grading: the Master curve's bottom point is the only knob the grade moves that no tool
// exposes, so the toe-to-floor relationship has only ever been DERIVED - and both attempts at the blue
// blacks were reverted for it (0.1.81, 0.1.86). This measures it: the bottom point at each x, the parade's
// three floors and the paired blacks cast read after each, the clip's own curves put back at the end.
// Prints a JSON row set to paste into src/lumetri_sweeps.json. It writes nothing the grade then reads.
// `curve` picks which one: "Master" moves all three channels together (measured 2026-09-17: a rigid
// translation, so it can set a black point but can never close a gap between the channel floors), "Red" /
// "Green" / "Blue" move one, which is the only thing that changes their spacing - the move neutralBottoms
// makes, and the one that has never been measured.
const CURVE_TOE_STEPS = [0, 0.02, 0.05, 0.1, 0.15, 0.2];
// "Luma vs Sat" is the shadow saturation roll-off, and it is the one write in the grade's first batch with
// NO model at all: the chain of 19:11 read 14.1 -> 11.2 black balance -> 9.58 curve -> 9.01 sliders -> 11
// read, so every modelled step agreed and 1.99 entered somewhere unmodelled. Swept here as a DEPTH, the
// same number ROLLOFF_DEPTH sets (0.35 today), through satRolloff's own shape.
const CURVE_NAMES = ["Master", "Red", "Green", "Blue", "RGB", "Luma vs Sat", "Shadows luma", "Highlights pad", "Shadows pad"];
// A wheel PAD is (hue, sat) at luma 0.5. Neutral is sat 0, always the first row. The pass writes pads to
// 0.3 at most (MAX_SAT) and the nudge to 0.5, so 0.6 is past anything it reaches.
const PAD_SAT_STEPS = [0, 0.1, 0.2, 0.3, 0.45, 0.6];
...
async function curveSweepTool({ seconds, track = 1, points = null, curve = "Master", lift = false, anchor = null, hue = 0, masterBlack = null, keepFrames = false } = {}) {
  const at = Number(seconds);
  ...
  if (!which) return { text: "CLAUDE_FOR_ADOBE_ERROR:curve must be one of " + CURVE_NAMES.join(", "), isError: true };
  const isSat = which === "Luma vs Sat";
  // A wheel PAD: one wheel's (hue, sat) at luma 0.5, sat stepped, hue fixed per call. The last control the
  // pass writes with no pixel form (2026-09-18 17:17: the two clips still carrying a [table] tag both wrote a
  // Highlights pad). The rows carry every PAIRED band level per channel - blacks, shadows, midtones,
  // highlights, whites - because a pad is a chroma push over a band of levels, and the input-level overlay
  // that found the last three forms needs several levels per channel, not the two (p1, p99) the 2026-09-15
  // `wheelBands` sweep recorded. Hue convention (QE): 0 red, 90 green, 180 cyan, 270 magenta.
  const isPad = which === "Highlights pad" || which === "Shadows pad";
  const padWheel = which === "Highlights pad" ? "highlights" : "shadows";
  const padHue = ((Number(hue) % 360) + 360) % 360;
  // "Shadows luma" is the Shadows colour wheel's luma slider - the closest thing Lumetri has to a
  // colorist's Lift, and the one black-end control the grade has never used (the skill rules it out as
  // "the wrong tool showing"). The 2026-09-15 `wheels` sweep says it moves the black point as far as the
  // Shadows SLIDER for a third of the midtone drag, but it recorded no floor column, so whether it CLIPS
  // - the thing that decides if it buys headroom a curve toe cannot - has never been measured.
  const isWheelLuma = which === "Shadows luma";
  let xs = (Array.isArray(points) && points.length ? points : isSat ? ROLLOFF_STEPS : isWheelLuma ? WHEEL_LUMA_STEPS : isPad ? PAD_SAT_STEPS : CURVE_TOE_STEPS).map(Number).filter((x) => x >= 0 && x <= (isWheelLuma || isPad ? 1 : 0.5));
  if (isPad && !xs.some((x) => x < 1e-9)) xs = [0, ...xs]; // neutral (sat 0) is the baseline row, always
  // The wheel's luma is 0..1 with NEUTRAL AT 0.5, not 0 - a different scale from every other sweep here,
  // and the 20:17 run swept 0..0.2 because the caller reused the curve steps: every row sat below neutral,
  ...
  const card = addTool(which + (isWheelLuma || isPad ? "" : lift ? " lift" : " toe") + (isPad ? " hue " + padHue : "") + (isFinite(anchor) && anchor > 0 && anchor < 1 ? " anchored at " + anchor : "") + " sweep at " + at + "s: " + xs.join(", "), "");
  setStatus("Sweeping the " + which + (isWheelLuma || isPad ? "" : lift ? " lift" : " toe") + "…");
  let copyNote = "";
  try { copyNote = await ensureWorkingCopy(); } catch (error) { return err(card, "Could not duplicate the sequence before editing: " + error.message); }
  const cw = isSat ? satWriter(at, track) : isWheelLuma || isPad ? wheelWriter(at, track) : curveWriter(at, track);
  let before, clipName = "", rows = [];
  // The bottom point on one curve, everything else left exactly as the clip had it. Two directions, and
  // they are different moves: a TOE sets the bottom point's INPUT ([[x, 0]]) and pulls the channel down,
  // clipping everything below x to zero; a LIFT sets its OUTPUT ([[0, y]]) and raises the channel's floor
  // without clipping anything. Only the toe has ever been measured (`channelToe`), which is why the grade
  // only pulls down - and why a black balance spends the floor headroom the black point then needs.
  // `anchor` pins the curve at a luma the picture should keep (0..1) and at 0.8, which is what the GRADE
  // actually writes - levelsFor passes the frame's median. curveToe was swept UNANCHORED, i.e. a two-point
  // line, so the anchored 4-point spline Premiere draws for the real thing has never been measured. The
  // 19:11 chain is off by 1.99 with every modelled step agreeing, and this is the difference left.
  const anch = isFinite(anchor) && anchor > 0 && anchor < 1 ? Number(anchor) : null;
  // A channel curve takes the same anchor form as Master (pinned at the anchor and at 0.8), and "RGB" writes
  // all three channels at one bottom point in ONE write - the pass's candidate replacement for the Master
  // bottom point, since 2026-09-18 21:35 showed the Master curve is not three channel curves on green
  // while each channel curve alone is the per-channel line pixel-exact.
  const chanPts = (v) => (v > 0.002 ? (lift ? [[0, v], [1, 1]] : anch && anch > v + 0.05 ? [[v, 0], [anch, anch], [0.8, 0.8], [1, 1]] : [[v, 0], [1, 1]]) : [[0, 0], [1, 1]]);
  const toe = (v) => (which === "Master" ? (lift ? Object.assign({}, before, { Master: v > 0.002 ? [[0, v], [1, 1]] : [[0, 0], [1, 1]] }) : curveLevels(v, 1, before, anch))
    : which === "RGB" ? Object.assign({}, before, { Red: chanPts(v), Green: chanPts(v), Blue: chanPts(v) })
    : Object.assign({}, before, { [which]: chanPts(v) }));
  // `masterBlack`: sweep the wheel UNDER the anchored Master curve the grade writes before it. Every wheel-
  // luma sweep on file is on a bare clip, and on the sixth live run (2026-09-18) the two chains whose lift
  // read wrong (C187 +1.5, C227 +1.6) both had a 0.02-0.09 anchored curve under the wheel, with the OPPOSITE
  // sign to the bare-clip residual - so the interaction, not the wheel, is the open question. The curve is
  // written first, the baseline row (x 0.5) reads it alone, and both are put back.
  const under = isWheelLuma && isFinite(masterBlack) && masterBlack > 0 && masterBlack <= 0.5 ? Number(masterBlack) : null;
  const curveW = under ? curveWriter(at, track) : null;
  let curvesBefore = null;
  try {
    const read = await cw.read();
    before = isSat ? read.points : isWheelLuma || isPad ? read.wheels : read.curves; clipName = read.clipName;
    if (under) { curvesBefore = (await curveW.read()).curves; await curveW.write(curveLevels(under, 1, curvesBefore, anch)); }
    for (const x of xs) {
      const { text } = await cw.write(isSat ? (x > 0.002 ? curveSatRolloff({ depth: x }) : [])
        : isWheelLuma ? Object.assign({}, before, { shadows: Object.assign({}, (before && before.shadows) || { hue: 0, sat: 0 }, { luma: x }) })
        : isPad ? Object.assign({}, before, { [padWheel]: { hue: padHue, sat: x, luma: 0.5 } })
        : toe(x));
      const m = await measureFrameAt(at, { region: "frame", keepPlayhead: true, keep: !!keepFrames });
      // keepFrames: every row's rendered PNG kept at a stable path, for a per-pixel comparison offline
      // (tools/pixel_map.cjs). The statistics above are percentiles; a percentile cannot say whether one
      // channel's output depends on another channel's input, and on 2026-09-18 21:16 the Master curve was
      // found to move green off the per-channel line while the Green curve alone sits on it.
      // 21:35: two Master sweeps in one paste overwrote each other; the name carries the time, the curve under, and the anchor.
      let kept = null;
      if (keepFrames && m.src) { kept = path.join(os.tmpdir(), "claude-for-adobe-sweep-" + String(at).replace(".", "p") + "s-" + which.replace(/\W+/g, "_") + (under ? "-under" + String(under).replace(".", "p") : "") + (anch ? "-a" + String(anch).replace(".", "p") : "") + "-" + String(x).replace(".", "p") + ".png"); try { fs.renameSync(m.src, kept); } catch (_) { try { fs.copyFileSync(m.src, kept); fs.rmSync(m.src, { force: true }); } catch (_) { kept = null; } } }
      const b = (m.bands && m.bands.blacks) || {};
      const lv = b.levels || {};
      if (isPad) {
        // Every paired band level per channel, plus the channel ends and means: the input-level overlay
        // needs as many (level -> delta) pairs per channel as the frame will give.
        const band = (name) => ((m.bands && m.bands[name] && m.bands[name].levels) || {});
        const row = { sat: x, hue: padHue, wheel: padWheel, wrote: text, lumaP1: round2(m.luma.p1), lumaP50: round2(m.luma.p50), lumaP99: round2(m.luma.p99), lumaMax: round2(m.luma.max) };
        for (const ch of ["red", "green", "blue"]) {
          row[ch + "P1"] = round2(m[ch].p1); row[ch + "P99"] = round2(m[ch].p99); row[ch + "Mean"] = round2(m[ch].mean);
          for (const name of ["blacks", "shadows", "midtones", "highlights", "whites"]) { const v = band(name)[ch]; if (isFinite(v)) row[ch + "_" + name] = round2(v); }
          row["clip" + ch[0].toUpperCase() + ch.slice(1)] = m.clipped[ch]; row["floor" + ch[0].toUpperCase() + ch.slice(1)] = m.floor[ch];
        }
        row.whitesRB = m.bands && m.bands.whites ? m.bands.whites.rb : null; row.blacksRB = b.rb; row.satP50 = m.saturation.p50;
        rows.push(row);
        continue;
      }
      ...
      rows.push({ x, wrote: text, lumaP1: round2(m.luma.p1), lumaP50: round2(m.luma.p50), lumaP99: round2(m.luma.p99), redP1: round2(m.red.p1), greenP1: round2(m.green.p1), blueP1: round2(m.blue.p1),
        ...
        pairedRed: lv.red, pairedGreen: lv.green, pairedBlue: lv.blue,
        floorRed: m.floor.red, floorGreen: m.floor.green, floorBlue: m.floor.blue });
      if (kept) rows[rows.length - 1].frame = kept;
    }
  } catch (error) { try { if (before) await cw.write(before); if (curvesBefore) await curveW.write(curvesBefore); } catch (_) {} return err(card, error.message); }
  try { await cw.write(before); if (curvesBefore) await curveW.write(curvesBefore); } catch (error) { rows.push({ restoreFailed: error.message }); }
  if (isPad) {
    const ok = rows.filter((r) => !r.restoreFailed);
    const lines = [
      "CHECK PASS: swept the " + which + " (" + padWheel + " wheel, hue " + padHue + ", luma 0.5) on " + clipName + " at " + at + "s, " + ok.length + " sats, put back.",
      "sat → luma p1 / p50 / p99 / max | channel p99 R/G/B | channel mean R/G/B | paired whites R/G/B | paired highlights R/G/B | paired midtones R/G/B | whites B-R | clipped R/G/B %",
      ...ok.map((r) => "  " + r.sat + " → " + r.lumaP1 + " / " + r.lumaP50 + " / " + r.lumaP99 + " / " + r.lumaMax + " | " + r.redP99 + "/" + r.greenP99 + "/" + r.blueP99 + " | " + r.redMean + "/" + r.greenMean + "/" + r.blueMean +
        " | " + r.red_whites + "/" + r.green_whites + "/" + r.blue_whites + " | " + r.red_highlights + "/" + r.green_highlights + "/" + r.blue_highlights + " | " + r.red_midtones + "/" + r.green_midtones + "/" + r.blue_midtones +
        " | " + r.whitesRB + " | " + r.clipRed + "/" + r.clipGreen + "/" + r.clipBlue),
      "The clip's wheels before the sweep (put back): " + JSON.stringify(before),
      "Paste as a `" + padWheel + "Pad` block in src/lumetri_sweeps.json (one block per hue; the rows carry hue and wheel):",
      JSON.stringify(ok.map(({ wrote, ...r }) => r)),
    ];
    if (rows.some((r) => r.restoreFailed)) lines.push("WARNING: the wheels were NOT restored (" + rows.find((r) => r.restoreFailed).restoreFailed + ") — Cmd+Z.");
    card.done(lines.join("\n"), true);
    setStatus("Thinking…");
    return { text: copyNote + lines.join("\n") };
  }
  const lines = [
    "CHECK PASS: swept " + (isWheelLuma ? "the Shadows wheel's luma slider (0.5 neutral, down darkens the shadow end)" + (under ? " UNDER a Master curve black " + under + (anch ? " anchored at " + anch : " unanchored") + " (written first, the 0.5 row reads it alone, put back with the wheels)" : "") : "the " + which + " curve's bottom point " + (lift ? "UP (output lift, [[0,y]]: raises the floor, clips nothing)" : "DOWN (input toe, [[x,0]]: pulls the channel down, clips below x)")) + " on " + clipName + " at " + at + "s, " + rows.length + " settings, curves restored.",
    ...
    ...(keepFrames ? ["Frames kept: " + rows.filter((r) => r.frame).map((r) => r.x + " " + r.frame).join(", ")] : []),
    ...
    "Paste as a " + (isWheelLuma ? "shadowsWheelLuma" : isSat ? "satRolloffBlacks" : lift ? "channelLift" : which === "Master" ? "curveToe" : "channelToe") + " block in src/lumetri_sweeps.json:",
    JSON.stringify(rows.filter((r) => !r.restoreFailed).map(({ wrote, ...r }) => r)),
  ];
  if (rows.some((r) => r.restoreFailed)) lines.push("WARNING: the curves were NOT restored (" + rows.find((r) => r.restoreFailed).restoreFailed + ") — Cmd+Z.");
  card.done(lines.join("\n"), true);
  setStatus("Thinking…");
  return { text: copyNote + lines.join("\n") };
}

// CALIBRATION, not grading: one Lumetri SLIDER stepped through raw values, the frame read after each, the
// slider put back. The gap it fills is the one every block in src/lumetri_sweeps.json declares - every
// model in the pass is fitted on a single frame (C220 @0.5s), and nothing has ever been swept on a second
// one, because no tool could set a raw slider value: `grade` and `grade_shot` steer a STATISTIC to a
// target, which is the opposite of what a sweep needs.
//
// Rows carry the same columns as curve_sweep so the two can be read side by side.
const SLIDER_STEPS = { default: [-100, -50, -20, 0, 20, 50, 100], exposure: [-2, -1, -0.5, 0, 0.5, 1, 2], saturation: [0, 50, 100, 150, 200] };
async function sliderSweepTool({ seconds, track = 1, slider, points = null, region = "frame" } = {}) {
  const at = Number(seconds);
  if (!(at >= 0)) return { text: "CLAUDE_FOR_ADOBE_ERROR:seconds required (the timeline position to sweep on)", isError: true };
  const key = Object.keys(GRADE_PARAMS).find((k) => k.toLowerCase() === String(slider).toLowerCase());
  if (!key) return { text: "CLAUDE_FOR_ADOBE_ERROR:unknown slider " + slider + "; known: " + Object.keys(GRADE_PARAMS).join(", "), isError: true };
  const spec = GRADE_PARAMS[key];
  const neutral = isFinite(spec.neutral) ? spec.neutral : 0;
  let xs = (Array.isArray(points) && points.length ? points : SLIDER_STEPS[key] || SLIDER_STEPS.default)
    .map(Number).filter((v) => isFinite(v) && v >= spec.range[0] && v <= spec.range[1]);
  if (!xs.length) return { text: "CLAUDE_FOR_ADOBE_ERROR:points must be values inside " + key + "'s range " + spec.range.join(" to "), isError: true };
  // A sweep without its own baseline is not a sweep - the same rule curve_sweep learned the hard way.
  if (!xs.some((v) => Math.abs(v - neutral) < 1e-9)) xs = [neutral, ...xs];
  const card = addTool(key + " sweep at " + at + "s: " + xs.join(", "), "");
  setStatus("Sweeping " + key + "…");
  let copyNote = "";
  try { copyNote = await ensureWorkingCopy(); } catch (error) { return err(card, "Could not duplicate the sequence before editing: " + error.message); }
  const w = lumetriWriter(at, track, spec.lumetri, region);
  ...
  try {
    ...
    for (const v of xs) {
      ...
      const b = (m.bands && m.bands.blacks) || {}, lv = b.levels || {};
      rows.push({ value: v, lumaP1: round2(m.luma.p1), lumaP50: round2(m.luma.p50), lumaP99: round2(m.luma.p99), lumaMax: round2(m.luma.max),
        redP1: round2(m.red.p1), greenP1: round2(m.green.p1), blueP1: round2(m.blue.p1), redP99: round2(m.red.p99), greenP99: round2(m.green.p99), blueP99: round2(m.blue.p99),
        blacksRB: b.rb, readable: b.readable, pairedRed: lv.red, pairedGreen: lv.green, pairedBlue: lv.blue,
        satP50: m.saturation && m.saturation.p50, floorRed: m.floor.red, floorGreen: m.floor.green, floorBlue: m.floor.blue,
        clipRed: m.clipped.red, clipGreen: m.clipped.green, clipBlue: m.clipped.blue });
    }
  } catch (error) { try { if (before !== null) await w.set(before); } catch (_) {} return err(card, error.message); }
  let restored = true;
  try { await w.set(before); } catch (_) { restored = false; }
  const lines = [
    "CHECK PASS: swept " + key + " (Lumetri \"" + spec.lumetri + "\") on " + w.clipName() + " at " + at + "s, " + rows.length + " settings" + (restored ? ", put back to " + round2(before) + "." : " — NOT PUT BACK, Cmd+Z."),
    "value → luma p1 / median / p99 / max | p1 R/G/B | paired bottoms R/G/B | cast B-R [readable %] | sat p50 | floor R/G/B % | clipped R/G/B %",
    ...rows.map((r) => "  " + r.value + " → " + r.lumaP1 + " / " + r.lumaP50 + " / " + r.lumaP99 + " / " + r.lumaMax + " | " + r.redP1 + "/" + r.greenP1 + "/" + r.blueP1 +
      " | " + r.pairedRed + "/" + r.pairedGreen + "/" + r.pairedBlue + " | " + r.blacksRB + " [" + r.readable + "%] | " + r.satP50 +
      " | " + r.floorRed + "/" + r.floorGreen + "/" + r.floorBlue + " | " + r.clipRed + "/" + r.clipGreen + "/" + r.clipBlue),
    "The clip's " + key + " before the sweep: " + round2(before),
    "Paste into src/lumetri_sweeps.json beside the existing `" + key + "` block, as a SECOND frame — the point is how far the first frame's model transfers:",
    JSON.stringify(rows),
  ];
  card.done(lines.join("\n"), true);
  setStatus("Thinking…");
...
// A long grade must never outlast the caller's patience: the run of 19:12 took 515 s (Premiere's renders
// creep through a session, and the machine was busy) and the agent gave up waiting while the panel carried
// on and finished. So the pass works to a time budget and says exactly how to continue. The shot-match
// state is kept per sequence between calls, or a resumed run would grade a later cut of an already-graded
// file from scratch - the very thing the match exists to prevent.
const gradeMatched = new Map();
async function gradeSequenceTool({ track = 1, region = "subject", tolerance, read = "auto", confirm = true, start_at = 0, budget_seconds = 150, log: logArg = "ask", seconds } = {}) {
  ...
  const card = addTool("grade " + (seconds !== undefined && seconds !== null ? "clip @" + seconds + "s" : "sequence") + " V" + track + " (" + region + ")", "");
  ...
  if (seconds !== undefined && seconds !== null) {
    const at = Number(seconds);
    const one = timelineOrder.filter((c) => at >= c.start && at < c.end);
    if (!one.length) return err(card, "no footage at " + at + "s on V" + track + " (the track runs " + round2(timelineOrder[0].start) + "-" + round2(timelineOrder[timelineOrder.length - 1].end) + "s)");
    timelineOrder = one;
  }
  ...
  for (const c of clips) {
    ...
    if (budget_seconds > 0 && (Date.now() - t0) / 1000 > budget_seconds) { resumeAt = Math.round(c.start * 1000) / 1000; break; }
    ...
    if (readFrom === "source" && !graded && gradeLooksLikeLog(m)) {
      ...
      if (/^lut(-source)?$/.test(logMode)) {
        ...
        m = await timed(() => measureFrameAt(at, { region, keepPlayhead: true }), "render"); renders++;
        readFrom = "premiere";
      } else {
          ...
          try {
            if (skinNow.region === skinRegion) {
              ...
              if (!sk) { const off = Math.round((hueNow - GRADE_SKIN_HUE_TARGET) * 10) / 10;
                ...
                // it rotates one slice of the hue wheel and leaves the rest of the picture alone. Masks were
                // the other candidate and are unreachable - Adobe's engineer: "There is no supported
                // ExtendScript API to either apply effects, or modify masks", and UXP's entire mask surface
                // is hasObjectMask() (researched 15:30).
                const hc = hueCurveWriter(at, track, "Hue vs Hue");
                const centre = key.key.H[0];              // the skin's own hue, from Vision's boxes
                const hue0 = GRADE_STATS.skinHue(skinNow), target = gradeSkinTargetFor(hue0); // 123 from the red side, 132 from the yellow side
                ...
                for (let t = 0; t < 3; t++) {
                  ...
                  const next = Math.max(-SKIN_HUE_CAP, Math.min(SKIN_HUE_CAP, shift * Math.max(-2, Math.min(5, k))));
                  if (Math.abs(next - shift) < 0.002) break;
                  shift = next;
                }
                // A move that made it worse is not kept: the curve goes back to empty.
                ...
                const satAfter = round2(after.saturation.p50);
                const onLine = hueLast >= GRADE_SKIN_HUE[0] && hueLast <= GRADE_SKIN_HUE[1];
                parts.push("skin: " + boxes.length + " " + skinRegion + " hue " + round2(hue0) + "° → Hue vs Hue at " + centre.toFixed(2) + " (" + notes.join(", ") + ") → hue " + hueLast + "°, saturation " + satAfter + (onLine ? " ✓" : " (line 116-126°)"));
...
const TOOL_DEFS = [
  ...
  { name: "slider_sweep", description: "CALIBRATION, not grading. Steps ONE Lumetri slider through raw values, reads the whole frame after each, and puts the slider back. This is the only way to set a raw slider value — grade and grade_shot steer a statistic to a target, which is the opposite of what a sweep needs. Every calibration model in the pass is fitted on a single frame, so the highest-value use is sweeping a slider on a DIFFERENT clip and comparing with the existing block of the same name in src/lumetri_sweeps.json. One render per setting. The neutral value is always included as the baseline row.", inputSchema: { type: "object", properties: { seconds: { type: "number", description: "Timeline position: picks the clip and the frame that is measured." }, slider: { type: "string", enum: ["temperature", "tint", "exposure", "contrast", "highlights", "shadows", "whites", "blacks", "saturation", "vibrance"] }, points: { type: "array", items: { type: "number" }, description: "Raw slider values to try, in order. Defaults: -100,-50,-20,0,20,50,100 for most; -2,-1,-0.5,0,0.5,1,2 for exposure; 0,50,100,150,200 for saturation. Must be inside the slider's own range." }, track: { type: "number", description: "1-based video track, default 1." } }, required: ["seconds", "slider"] } },
  { name: "curve_sweep", description: "CALIBRATION, not grading. Measures what the Master curve's bottom point does to the parade: writes the bottom point at each x in turn, reads the whole frame after each (luma p1, the three channel p1s, the paired blacks cast of the darkest 3%, and how much of each channel is on the floor), then puts the clip's own curves back. One render per setting. Use it on a clip whose Lumetri is neutral, and paste the JSON it prints into src/lumetri_sweeps.json. Nothing in the grade reads it until it is pasted. Master moves all three channels together (measured: a rigid translation, so it sets a black point but cannot change the spacing between the channel floors); Red/Green/Blue move one channel's toe, which is the only move that changes that spacing.",     inputSchema: { type: "object", properties: { seconds: { type: "number", description: "Timeline position: picks the clip and the frame that is measured." }, curve: { type: "string", enum: ["Master", "Red", "Green", "Blue", "RGB", "Luma vs Sat", "Shadows luma", "Highlights pad", "Shadows pad"], description: "Which curve's bottom point to move. Default Master. 'RGB' writes the Red, Green and Blue curves at the same bottom point in one write (not Master). 'Highlights pad' / 'Shadows pad' sweep that colour wheel's PAD instead: (hue, sat) at luma 0.5, `points` are sat values (default 0, 0.1, 0.2, 0.3, 0.45, 0.6; sat 0 is neutral and always the first row), `hue` fixes the direction. Rows carry every paired band level per channel - the input-level overlay that fits a pixel form needs several levels per channel." }, hue: { type: "number", description: "For a pad sweep only: the wheel hue in degrees, QE convention 0 red, 90 green, 180 cyan, 270 magenta. Default 0. Sweep at least two hues (0 and 90) so the per-channel weights can be separated from the band shape." }, points: { type: "array", items: { type: "number" }, description: "Positions to try, in order — and the scale DEPENDS ON `curve`. Master/Red/Green/Blue: a bottom point 0-0.5, neutral 0, default 0, 0.02, 0.05, 0.10, 0.15, 0.20. \"Luma vs Sat\": a roll-off depth 0-0.5, neutral 0, default 0, 0.1, 0.2, 0.35, 0.5. \"Shadows luma\": the wheel slider 0-1 with NEUTRAL AT 0.5 and down darkening, default 0.5, 0.45, 0.4, 0.35, 0.3, 0.25 — omit this argument unless you mean a different range. The baseline row is always included." }, anchor: { type: "number", description: "Pin the Master curve at this luma (0-1, e.g. the frame's median 0.5) and at 0.8, which is the 4-point spline the grade actually writes. Omit for the plain two-point line. Master, Red/Green/Blue/RGB (same four-point form on that channel), or with `masterBlack`." }, keepFrames: { type: "boolean", description: "Keep every row's rendered PNG in the temp folder and print the paths, for a per-pixel comparison offline (tools/pixel_map.cjs). Default false." }, masterBlack: { type: "number", description: "\"Shadows luma\" only: write a Master curve with this bottom point (0-0.5, with `anchor` if given - the curve the grade writes) BEFORE sweeping the wheel, so the rows measure the wheel on top of the curve; the 0.5 row reads the curve alone. Both are put back." }, lift: { type: "boolean", description: "Sweep the bottom point UP instead of down: its output ([[0,y]]), which raises that channel's floor and clips nothing, rather than its input ([[x,0]]), which pulls the channel down and clips below x. Default false." }, track: { type: "number", description: "1-based video track, default 1." } }, required: ["seconds"] } },
