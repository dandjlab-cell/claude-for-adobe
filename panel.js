...
async function curveSweepTool({ seconds, track = 1, points = CURVE_TOE_STEPS, curve = "Master" } = {}) {
  const at = Number(seconds);
  ...
  const card = addTool(which + " toe sweep at " + at + "s: " + xs.join(", "), "");
  setStatus("Sweeping the " + which + " toe…");
  let copyNote = "";
  try { copyNote = await ensureWorkingCopy(); } catch (error) { return err(card, "Could not duplicate the sequence before editing: " + error.message); }
  const cw = curveWriter(at, track);
  let before, clipName = "", rows = [];
  // The bottom point on one curve, everything else left exactly as the clip had it.
  const toe = (x) => (which === "Master" ? curveLevels(x, 1, before, null) : Object.assign({}, before, { [which]: x > 0.002 ? [[x, 0], [1, 1]] : [[0, 0], [1, 1]] }));
  try {
  ...
  const lines = [
    "CHECK PASS: swept the " + which + " curve's bottom point on " + clipName + " at " + at + "s, " + rows.length + " settings, curves restored.",
    ...
    "Paste as a " + (which === "Master" ? "curveToe" : "channelToe") + " block in src/lumetri_sweeps.json:",
