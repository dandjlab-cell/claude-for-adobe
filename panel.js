...
const CURVE_NAMES = ["Master", "Red", "Green", "Blue", "Luma vs Sat"];
...
async function curveSweepTool({ seconds, track = 1, points = CURVE_TOE_STEPS, curve = "Master", lift = false, anchor = null } = {}) {
  ...
  const xs = (Array.isArray(points) && points.length ? points : isSat ? ROLLOFF_STEPS : CURVE_TOE_STEPS).map(Number).filter((x) => x >= 0 && x <= 0.5);
  ...
  let copyNote = "";
  try { copyNote = await ensureWorkingCopy(); } catch (error) { return err(card, "Could not duplicate the sequence before editing: " + error.message); }
  const cw = isSat ? satWriter(at, track) : curveWriter(at, track);
  ...
  try {
    ...
    before = isSat ? read.points : read.curves; clipName = read.clipName;
    for (const x of xs) {
      const { text } = await cw.write(isSat ? (x > 0.002 ? curveSatRolloff({ depth: x }) : []) : toe(x));
  ...
  const lines = [
    ...
    "The clip's " + (isSat ? "Luma vs Sat" : "curves") + " before the sweep (put back): " + (isSat ? (before.length ? JSON.stringify(before) : "empty") : curvesIdentity(before) ? "identity" : JSON.stringify(before)),
    "Paste as a " + (isSat ? "satRolloffBlacks" : lift ? "channelLift" : which === "Master" ? "curveToe" : "channelToe") + " block in src/lumetri_sweeps.json:",
