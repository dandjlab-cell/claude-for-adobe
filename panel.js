...
async function reframeTool({ aspect, preset, width, height, fps, bin, name, reframe = "fill", motion, max = 8, max_px = 512 } = {}) {
  const parts = [], content = [];
  let step;
  // From a bin the sequence is built WITHOUT tracking unless asked by name: Auto Reframe on the raw length is the
...
const TOOL_DEFS = [
  ...
  { name: "reframe", description: "THE call for any shape request: 'make it 9:16', '4:5', '16:9 version', or '9:16 from this bin'. One deterministic pass: from a bin, a sequence at the asked shape is built straight from the raw footage (never an intermediate shape); an open timeline is resized on its working copy. Footage fills the frame and is centred, graphics/titles/guides keep their placement, then Premiere's own Auto Reframe effect goes on every footage clip (Premiere analyses each clip's SOURCE and follows the subject inside the frame). Returns the visible moments and the seams as frames with CHECK lines. Afterwards: judge the picture in each frame, nudge_clip (with track) only what is wrong, snapshot_moments once more. motion 'static' = fill-and-centre only, no tracking.",
    inputSchema: { type: "object", properties: { aspect: { type: "string", description: "9:16, 4:5, 1:1, 16:9, 2.39:1 ..." }, preset: { type: "string", enum: ["vertical", "hd", "uhd", "square", "four_five"] }, width: { type: "number" }, height: { type: "number" }, fps: { type: "number" }, bin: { type: "string", description: "bin path of raw footage to build a NEW sequence from; omit to reframe the open timeline" }, name: { type: "string", description: "name for the new sequence (with bin); default '<bin> <aspect>'" }, motion: { type: "string", enum: ["track", "static"], description: "on an open timeline: track (default), Premiere Auto Reframe follows the subject; from a bin: static (default), fill and centre, and tracking is applied by a second reframe call after the cut" }, reframe: { type: "string", enum: ["fill", "fit", "none"], description: "static mode only: fill (default) or fit" }, max: { type: "number", description: "moments to render, default 8" } } } },
