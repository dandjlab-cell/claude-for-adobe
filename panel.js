...
const CURVE_NAMES = ["Master", "Red", "Green", "Blue"];
async function curveSweepTool({ seconds, track = 1, points = CURVE_TOE_STEPS, curve = "Master", lift = false } = {}) {
  ...
  const xs = (Array.isArray(points) && points.length ? points : CURVE_TOE_STEPS).map(Number).filter((x) => x >= 0 && x <= 0.5);
  ...
  try {
    ...
    before = read.curves; clipName = read.clipName;
    for (const x of xs) {
      const { text } = await cw.write(toe(x));
  ...
  const lines = [
    ...
    "The clip's curves before the sweep (put back): " + (curvesIdentity(before) ? "identity" : JSON.stringify(before)),
    "Paste as a " + (lift ? "channelLift" : which === "Master" ? "curveToe" : "channelToe") + " block in src/lumetri_sweeps.json:",
