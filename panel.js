...
async function curveSweepTool({ seconds, track = 1, points = null, curve = "Master", lift = false, anchor = null } = {}) {
  ...
  let xs = (Array.isArray(points) && points.length ? points : isSat ? ROLLOFF_STEPS : isWheelLuma ? WHEEL_LUMA_STEPS : CURVE_TOE_STEPS).map(Number).filter((x) => x >= 0 && x <= (isWheelLuma ? 1 : 0.5));
  ...
  let copyNote = "";
  try { copyNote = await ensureWorkingCopy(); } catch (error) { return err(card, "Could not duplicate the sequence before editing: " + error.message); }
  const cw = isSat ? satWriter(at, track) : isWheelLuma ? wheelWriter(at, track) : curveWriter(at, track);
  ...
  try {
    ...
    before = isSat ? read.points : isWheelLuma ? read.wheels : read.curves; clipName = read.clipName;
    for (const x of xs) {
