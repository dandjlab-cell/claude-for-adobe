...
async function curveSweepTool({ seconds, track = 1, points = CURVE_TOE_STEPS, curve = "Master", lift = false, anchor = null } = {}) {
  ...
  const xs = (Array.isArray(points) && points.length ? points : isSat ? ROLLOFF_STEPS : isWheelLuma ? WHEEL_LUMA_STEPS : CURVE_TOE_STEPS).map(Number).filter((x) => x >= 0 && x <= (isWheelLuma ? 1 : 0.5));
  if (!xs.length) return { text: "CLAUDE_FOR_ADOBE_ERROR:points must be curve x positions between 0 and 0.5", isError: true };
  const card = addTool(which + (lift ? " lift" : " toe") + (isFinite(anchor) && anchor > 0 && anchor < 1 ? " anchored at " + anchor : "") + " sweep at " + at + "s: " + xs.join(", "), "");
  ...
  const lines = [
    "CHECK PASS: swept the " + which + " curve's bottom point " + (lift ? "UP (output lift, [[0,y]]: raises the floor, clips nothing)" : "DOWN (input toe, [[x,0]]: pulls the channel down, clips below x)") + " on " + clipName + " at " + at + "s, " + rows.length + " settings, curves restored.",
