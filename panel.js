...
async function logLutTool({ action = "status", id, seconds, track = 1, where = "clip" } = {}) {
...
async function gradeSequenceTool({ track = 1, region = "subject", tolerance, read = "auto", confirm = true, start_at = 0, budget_seconds = 150, log: logArg = "ask" } = {}) {
  const log = logArg === "interpret" ? "premiere" : logArg; // the old name, kept working
  ...
  for (const c of clips) {
    ...
    if (readFrom === "source" && !graded && gradeLooksLikeLog(m)) {
      ...
      if (log === "ask") {
        ...
        lines.push(label + ": log, " + (hint || "maker not named by the file") + ". Which conversion - " + (hint ? hint + "'s own LUT (I download it)" : "the maker's own LUT") + " or Premiere's built-in transform? And where - on the clip (Lumetri's Input LUT, goes with Discard copy) or in the file's source settings (stays; every cut of the file gets it)? Premiere's transform is source-only. Then I rerun and grade in the same pass: log \"lut\" | \"lut-source\" | \"premiere\".");
        logSkipped++;
        continue;
      }
      if (log === "lut" || log === "lut-source") {
        const r = await logLutTool({ action: "use", seconds: at, track, where: log === "lut-source" ? "source" : "clip" });
        if (r.isError) { lines.push(label + ": " + logNote + ". " + r.text); logSkipped++; continue; }
        logPart = "log → the maker's LUT, " + (log === "lut-source" ? "in the file's source settings (stays after Discard)" : "on this clip (goes with Discard)");
