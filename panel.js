...
const CURVE_NAMES = ["Master", "Red", "Green", "Blue", "Luma vs Sat", "Shadows luma", "Highlights pad", "Shadows pad"];
...
async function curveSweepTool({ seconds, track = 1, points = null, curve = "Master", lift = false, anchor = null, hue = 0, masterBlack = null, keepFrames = false } = {}) {
  ...
  const toe = (v) => (which === "Master" ? (lift ? Object.assign({}, before, { Master: v > 0.002 ? [[0, v], [1, 1]] : [[0, 0], [1, 1]] }) : curveLevels(v, 1, before, anch))
    : Object.assign({}, before, { [which]: v > 0.002 ? (lift ? [[0, v], [1, 1]] : [[v, 0], [1, 1]]) : [[0, 0], [1, 1]] }));
  ...
  try {
    ...
    for (const x of xs) {
      ...
      let kept = null;
      if (keepFrames && m.src) { kept = path.join(os.tmpdir(), "claude-for-adobe-sweep-" + which.replace(/\W+/g, "_") + "-" + String(x).replace(".", "p") + ".png"); try { fs.renameSync(m.src, kept); } catch (_) { try { fs.copyFileSync(m.src, kept); fs.rmSync(m.src, { force: true }); } catch (_) { kept = null; } } }
