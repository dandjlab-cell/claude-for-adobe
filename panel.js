...
async function gradeSequenceTool({ track = 1, region = "subject", tolerance, read = "auto", confirm = true, start_at = 0, budget_seconds = 150, log = "ask" } = {}) {
  ...
  for (const c of clips) {
    ...
    if (readFrom === "source" && !graded && gradeLooksLikeLog(m)) {
      ...
      if (log === "ask") {
        ...
        lines.push(label + " [" + (m.region || region) + "] " + logNote + ", " + (hint ? "shot on " + hint : "maker unknown from the file's tags") + ". Two ways to convert it, both need your word: (1) Premiere's own colour management, set as the file's colour interpretation in Modify > Color - exact, covers every cut of the file, but it is a source setting and stays after Discard copy; (2) the maker's official conversion LUT, downloaded from their site and put on this clip - removed with Discard copy, and it only covers the clips it is applied to. Say which and I run this again with it: the conversion goes on first and the grade follows in the same pass, no second step (grade_sequence with log: \"lut\" or log: \"interpret\").");
        logSkipped++;
        continue;
      }
      if (log === "lut") {
        const r = await logLutTool({ action: "use", seconds: at, track });
        if (r.isError) { lines.push(label + ": " + logNote + ". " + r.text); logSkipped++; continue; }
        logPart = "log → the maker's official LUT, on this clip (Discard copy removes it)";
      ...
      logPart = ("log → " + conv.name + " (a source setting, at your word; " + conv.tried + " conversion" + (conv.tried > 1 ? "s" : "") + " tried" + (conv.hint ? ", " + conv.hint + " first from the file's tags" : "") + "; chosen by the picture: black " + round2(cm.luma.p1) + " / white " + round2(cm.luma.p99) + " / colour " + round2(cm.saturation.p99) + ")");
      }
    }
