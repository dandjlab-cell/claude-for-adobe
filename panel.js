...
async function curveSweepTool({ seconds, track = 1, points = CURVE_TOE_STEPS, curve = "Master", lift = false } = {}) {
  const at = Number(seconds);
  ...
  const card = addTool(which + (lift ? " lift" : " toe") + " sweep at " + at + "s: " + xs.join(", "), "");
  ...
  const toe = (v) => (which === "Master" ? (lift ? Object.assign({}, before, { Master: v > 0.002 ? [[0, v], [1, 1]] : [[0, 0], [1, 1]] }) : curveLevels(v, 1, before, null))
