...
async function curveSweepTool({ seconds, track = 1, points = CURVE_TOE_STEPS } = {}) {
  const at = Number(seconds);
  ...
  const card = addTool("curve toe sweep at " + at + "s: " + xs.join(", "), "");
  setStatus("Sweeping the curve toe…");
  let copyNote = "";
  try { copyNote = await ensureWorkingCopy(); } catch (error) { return err(card, "Could not duplicate the sequence before editing: " + error.message); }
  ...
  try {
    ...
    for (const x of xs) {
      const { text } = await cw.write(curveLevels(x, 1, before, null));
      ...
      rows.push({ x, wrote: text, lumaP1: round2(m.luma.p1), redP1: round2(m.red.p1), greenP1: round2(m.green.p1), blueP1: round2(m.blue.p1),
        blacksRB: b.rb, blacksG: b.g, floorRed: m.floor.red, floorGreen: m.floor.green, floorBlue: m.floor.blue });
    }
  ...
  const lines = [
    "CHECK PASS: swept the Master curve's bottom point on " + clipName + " at " + at + "s, " + rows.length + " settings, curves restored.",
    "x → luma p1 | red p1 / green p1 / blue p1 | blacks cast B-R / G (paired, darkest 3%) | on the floor R/G/B %",
    ...rows.filter((r) => !r.restoreFailed).map((r) => "  " + r.x + " → " + r.lumaP1 + " | " + r.redP1 + " / " + r.greenP1 + " / " + r.blueP1 +
      " | " + r.blacksRB + " / " + r.blacksG + " | " + r.floorRed + "/" + r.floorGreen + "/" + r.floorBlue),
    "The clip's curves before the sweep (put back): " + (curvesIdentity(before) ? "identity" : JSON.stringify(before)),
    "Paste as a curveToe block in src/lumetri_sweeps.json:",
